...
class StateCity extends Component {
  ...
  getCity(arrayIndex) {
    this.setState({
      city_array: s_a[arrayIndex + 1].split('|'),
    });
  }
  render() {
    return (
      <View>
        <View style={{flex: 2, alignSelf: 'center', flexDirection: 'row'}}>
          <Text style={{fontSize: 17, color: 'gray', alignSelf: 'center'}}>
            State *
          </Text>
        ...
        {this.state.city_array.length != 0 && (
          <View style={{flexDirection: 'row'}}>
            ...
            <Picker
              ...
              {this.state.city_array.map((item, index) => {
                return <Picker.Item label={item} value={index} key={index} />;
              })}
            </Picker>
          </View>
